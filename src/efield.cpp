...
void setupMatrix(Mat A)
{
    ...
    // 2D matrix
        ...
        for(int i2 = 0; i2 < g_nx; i2++) {
            ...
            // Center
            err = MatSetValue(A, II1(i1,i2), II1(i1,i2), diag, INSERT_VALUES);CHKERRV(err);
            
            // Sides
            if(II1(i1m,i2) >= 0)
                err = MatSetValue(A, II1(i1,i2), II1(i1m,i2), offdiag, INSERT_VALUES);CHKERRV(err);
            if(II1(i1p,i2) >= 0)
                err = MatSetValue(A, II1(i1,i2), II1(i1p,i2), offdiag, INSERT_VALUES);CHKERRV(err);
            if(II1(i1,i2m) >= 0)
                err = MatSetValue(A, II1(i1,i2), II1(i1,i2m), offdiag, INSERT_VALUES);CHKERRV(err);
            if(II1(i1,i2p) >= 0)
                err = MatSetValue(A, II1(i1,i2), II1(i1,i2p), offdiag, INSERT_VALUES);CHKERRV(err);
            
            // Diagonals
            if(II1(i1m,i2m) >= 0)
                err = MatSetValue(A, II1(i1,i2), II1(i1m,i2m), offdiag, INSERT_VALUES);CHKERRV(err);
            if(II1(i1m,i2p) >= 0)
                err = MatSetValue(A, II1(i1,i2), II1(i1m,i2p), offdiag, INSERT_VALUES);CHKERRV(err);
            if(II1(i1p,i2m) >= 0)
                err = MatSetValue(A, II1(i1,i2), II1(i1p,i2m), offdiag, INSERT_VALUES);CHKERRV(err);
            if(II1(i1p,i2p) >= 0)
                err = MatSetValue(A, II1(i1,i2), II1(i1p,i2p), offdiag, INSERT_VALUES);CHKERRV(err);
        }}
    }
...
void efield(double *rho, double *E)
{
    ...
    if(g_dimension == 1) {
        for(int i = 0; i < g_nx-1; i++) {
            temp[i] = 0.5 * (rho[IK(i,0)] + rho[IK(i,1)] / 3.0)
                    + 0.5 * (rho[IK(i+1,0)] - rho[IK(i+1,1)] / 3.0) - rho0;
        }
    }
    else {
        for(int i1 = 0; i1 < g_nx; i1++) {
        for(int i2 = 0; i2 < g_nx; i2++) {
            ...
            i1m = (i1 == 0) ? g_nx-1 : i1-1;
            i2m = (i2 == 0) ? g_nx-1 : i2-1;
            temp[II1(i1,i2)] = -rho0
    ...
    if(g_dimension == 1) {
        for(int i = 0; i < g_nx; i++) {
            double phi1 = (i == 0)      ? 0.0 : temp[i-1];
            double phi2 = (i == g_nx-1) ? 0.0 : temp[i];
            E[i] = (phi2 - phi1) / g_dx;
        }
        
        if(s_isEfieldTest) {
            double *phi = new double[g_ESize];
            phi[0] = 0.0;
            for(int i = 1; i < g_nx; i++) {
                phi[i] = temp[i-1];
            }
            outputEfield("efield.dat", g_nx, phi, E, NULL);
            delete[] phi;
        }
    }
    else {
        ...
        for(int i2 = 0; i2 < g_nx; i2++) {
            ...
            phi11 = (i1  == 0 && i2  == 0) ? 0.0 : temp[II1(i1,i2)];
            phi12 = (i1  == 0 && i2p == 0) ? 0.0 : temp[II1(i1,i2p)];
            phi21 = (i1p == 0 && i2  == 0) ? 0.0 : temp[II1(i1p,i2)];
            phi22 = (i1p == 0 && i2p == 0) ? 0.0 : temp[II1(i1p,i2p)];
        ...
        if(s_isEfieldTest) {
            ...
            for(int i2 = 0; i2 < g_nx; i2++) {
                if(i1 == 0 && i2 == 0)
                    continue;
                phi[II(i1,i2)] = temp[II1(i1,i2)];
            }}
            outputEfield("efield.dat", g_nx*g_nx, phi, &E[NII(0,0,0)], &E[NII(1,0,0)]);
            delete[] phi;
        }
    }
